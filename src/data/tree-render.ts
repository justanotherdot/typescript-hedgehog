...
function renderTreeRecursive<T>(
  ...
  for (let i = 0; i < childrenToShow; i++) {
    ...
    const isLast = i === childrenToShow - 1 && childrenToShow === tree.children.length;
    const childPrefix = prefix + (isLast ? "└── " : "├── ");
    const nextPrefix = prefix + (isLast ? "    " : "│   ");
    
    if (options.showIndices) {
      result += `${prefix}${isLast ? "└" : "├"}─[${i}]─ `;
      result += renderTreeRecursive(child, options, depth + 1, nextPrefix).slice(prefix.length + 6);
    } else {
 ...
 */
export function renderTreeCompact<T>(tree: Tree<T>, maxDepth: number = 3): string {
  return renderTreeCompactRecursive(tree, maxDepth, 0);
}

function renderTreeCompactRecursive<T>(tree: Tree<T>, maxDepth: number, depth: number): string {
  if (depth >= maxDepth) {
    return `${tree.value}(...)`;
  }
...
function getTreeStatsRecursive<T>(tree: Tree<T>, depth: number): TreeStats {
  ...
  for (const child of tree.children) {
    ...
    totalBranchingFactors += childStats.totalNodes * childStats.avgBranchingFactor;
  }
